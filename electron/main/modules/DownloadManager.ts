...
export class DownloadManager {
    ...
    addDownloadToQueue(file: FileDownload, start = true) {
        ...
        const dl = new EasyDl(url, join(config.destinationDir, file.name), {
            ...
            this._browserWindow.webContents.send('file-download-started', JSON.stringify({
                fileId: file.id,
            ...
            this._browserWindow.webContents.send('file-download-progress', JSON.stringify({
                fileId: file.id,
            ...
            downloaded = true;
            this._browserWindow.webContents.send('file-download-complete', JSON.stringify({
                fileId: file.id,
            ...
            if (!downloaded) {
                this._browserWindow.webContents.send('file-download-canceled', JSON.stringify({
                    fileId: file.id,
                }));
            }
        }).on("error", (err) => {
        ...
        dl.on('build',  (progress) => {
            this._browserWindow.webContents.send('file-download-building', JSON.stringify({
                fileId: file.id,
